...
IDS_MSG_UNREGISTERED_EXPIRE_VERSION = Unregister/Expired version*/
        ...
        public string UserName()
        {
            ...
            try
            {
                IntPtr ptrResult = ST_GetUserName();
                if (ptrResult != IntPtr.Zero)
                {
                    sRet = Marshal.PtrToStringAuto(ptrResult);
                    Marshal.Release(ptrResult);
            ...
            {
                cGlobalSettings.oLogger.WriteLogException("UserName()|error:{0}", oEx);
              //  //  oUtils.WriteToFile(string.Format("UserName()|error:{0}", oEx.Message));
            }
            return sRet;
        }
        ...
        {
            //return oArmadillo.DaysInstalled;//modified by anurag jain 2010-07-19 12-05
            //to be modified
            long nDaysInstalled = 0;
            try
            {
                nDaysInstalled = ST_GetInstalledDays();
            }
            catch (Exception oEx)
            {
                cGlobalSettings.oLogger.WriteLogException("DaysInstalled ::", oEx);
             //   //  oUtils.WriteToFile("DaysInstalled()", oEx.Message);
            }
            ...
            try
            {
             //   ST_GetUserKey2(sRet);
               ...
               if (string.IsNullOrEmpty(sRet) && IsRegistered())
               {
                   ...
                   // IntPtr ptrResult = ST_GetKey();
                   IntPtr ptrResult = ST_GetUserKey2();
                   if (ptrResult != IntPtr.Zero)
                   {
                       sRet = Marshal.PtrToStringAuto(ptrResult);
                       if (!string.IsNullOrEmpty(sRet))
        ...
        public long DaysLeft()
        {
            ...
            try
            {
                ...
                //nDaysLeft = ST_GetTotalAllowedTrialPeriod();
                nDaysLeft = ST_GetTotalAllowedTrialPeriod2();
            }
            catch (Exception oEx)
            {
                cGlobalSettings.oLogger.WriteLogException("DaysLeft()|error:{0}", oEx);
        ...
        public bool Expired()
        {
            ...
            try
            {
              ...
              int nRetVal =  ST_XGetRegStatus();

              eSYS_TRIAL_VERSION_STATUS oStatus = (eSYS_TRIAL_VERSION_STATUS)nRetVal;

              switch (oStatus)
              {
                  case eSYS_TRIAL_VERSION_STATUS.STATUS_EVAL_VERSION_EXPIRED:                     
                  case eSYS_TRIAL_VERSION_STATUS.STATUS_SUBSCRIPTION_EXPIRED:
                      bRet = true;
                      break;
                  case eSYS_TRIAL_VERSION_STATUS.STATUS_SUBSCRIPTION_ACTIVATION_FAILED:
                      bRet = false;
                      break;
                  case eSYS_TRIAL_VERSION_STATUS.STATUS_EVAL_PERIOD_REMAINING:                      
                  case eSYS_TRIAL_VERSION_STATUS.STATUS_SUBSCRIPTION_REMAINING:
                      bRet = false;
                      break;
                  default:
                      break;
        ...
        public void ResetKey()
        {
            try
            {
                ...
                try
                {
                    ...
                    string sPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), ProductName);
                ...
                {
                    cGlobalSettings.oLogger.WriteLogException("GetFolderPath - ResetKey ::", ex);
                    //  oUtils.WriteToFile(string.Format("SetExpireKey()|unable to delete files |error:{0}", ex.Message));
                }


                RegistryKey oReg = Registry.LocalMachine.OpenSubKey("software\\k9");
                if (oReg != null)
                {
                    RegistryKey SubKey = Registry.LocalMachine.OpenSubKey("software", true);
                    SubKey.DeleteSubKeyTree("k9");
                    SubKey.Close();
                    oReg.Close();
                }


                oReg = Registry.CurrentUser.OpenSubKey("software\\k9");
                if (oReg != null)
                {
            ...
            {
                cGlobalSettings.oLogger.WriteLogException("ResetKey ::", ex);
            }
        }

#endif
        public bool SetExpireKey()
        {
            bool bres = false;
            try
            {
        ...
        public bool InstallKey(string sName, string sKey, bool bInstallOnExit)
        {
            ...
            try
            {
                ...
                bRet = ST_InstallArmadilloCode2(/*sName,*/ sKey);
            }
            catch (Exception oEx)
            {
                cGlobalSettings.oLogger.WriteLogException("DaysInstalled ::", oEx);
...
#if false
        public bool InstallKeyNew(string sName, string sKey, bool bInstallOnExit)
        {
            //return oArmadillo.InstallKey(sName, sKey, bInstallOnExit);//modified by anurag jain 2010-07-19 12-05
            //to be modified
            bool bRet = false;
            ...
            {
                cGlobalSettings.oLogger.WriteLogException("InstallKey ::", oEx);
                //  oUtils.WriteToFile(string.Format("InstallKey()|error:{0}", oEx.Message));
            }
            return bRet;
        }
        ...
        public bool UninstallKey()
        {
            //return oArmadillo.UninstallKey();//modified by anurag jain 2010-07-19 12-05
            //to be modified
            bool bRet = false;
            try
            {
                bRet = ST_UnInstallKey();
            }
            catch (Exception oEx)
            {
                cGlobalSettings.oLogger.WriteLogException("UninstallKey ::", oEx);
                //  oUtils.WriteToFile(string.Format("UninstallKey()|error:{0}", oEx.Message));
            }
            return bRet;
        }
...
//                 if (bUpdateEnvironment) ST_UpdateEnviornment();
                ...
                bRet = ST_VerifyArmadilloKey2(sKey);   
            }
            catch (Exception oEx)
            {
                cGlobalSettings.oLogger.WriteLogException("IsValidKey ::", oEx);
        ...
        {
            //return oArmadillo.UpdateEnvironment();//commented by anurag jain 2010-07-19 12-05
            //to be modified
            bool bRet = false;
            try
            {
                bRet = ST_UpdateEnviornment();
            }
            catch (Exception oEx)
            {
                cGlobalSettings.oLogger.WriteLogException("UpdateEnvironment ::", oEx);
                //  oUtils.WriteToFile(string.Format("UpdateEnvironment()|error:{0}", oEx.Message));
            }
            return bRet;
        }
        ...
        /// </summary>
        /// <param name="nIndex">index of string to return</param>
        /// <param name="sUserString">buffer to hold the string</param>
        /// <param name="nLength">length of the string</param>
        /// <returns>true if successfull else false</returns>
        public bool GetUserString(int nIndex, out string sUserString, int nLength)
        {
            bool bRet = false;
            string sTempString = string.Empty;
            try
            {
                byte[] bByte = new byte[nLength];
                bRet = STGetUserString(nIndex, bByte, nLength);
                if (bRet)
                {
                    sTempString = Encoding.Default.GetString(bByte);
                    if (!string.IsNullOrEmpty(sTempString))
                    {
                        sTempString = sTempString.Trim(new char[] { '\0' });
                    }
                }
            }
            catch (Exception oEx)
            {
                cGlobalSettings.oLogger.WriteLogException("GetUserString ::", oEx);
                //  oUtils.WriteToFile(string.Format("GetUserString():error{0}", oEx.Message));
            }
            sUserString = sTempString;
            return bRet;
        }
        ...
        /// set the user string
        /// </summary>
        /// <param name="nIndex">index of the string to save</param>
        /// <param name="sUserString"></param>
        /// <returns>true if successfully set</returns>
        public bool SetUserString(int nIndex, string sUserString)
        {
            bool bRet = false;
            try
            {
                bRet = STSetUserString(nIndex, sUserString);
            }
            catch (Exception oEx)
            {
                cGlobalSettings.oLogger.WriteLogException("SetUserString ::", oEx);
                //  oUtils.WriteToFile(string.Format("SetUserString():error{0}", oEx.Message));
            }
            return bRet;
        }

        /// <summary>
        /// Get Total Allowed Trial Period
        /// </summary>
        /// <returns>no of trial days</returns>
        public long TotalAllowedTrialPeriod()
        {
            long nTotalAllowedTrialPeriod = 0;
            try
            {
                nTotalAllowedTrialPeriod = ST_GetTotalAllowedTrialPeriod();
            }
            catch (Exception oEx)
            {
                cGlobalSettings.oLogger.WriteLogException("TotalAllowedTrialPeriod ::", oEx);
                //  oUtils.WriteToFile(string.Format("TotalAllowedTrialPeriod()|error:{0}", oEx.Message));
            }
            return nTotalAllowedTrialPeriod;
        }

        /// <summary>
        /// get the value of environment variables
        /// </summary>
        /// <param name="sEnvName">Name of the environment variable whose value we have to retrieve</param>
        /// <returns>returns the value of the variable</returns>
        public string GetEnvironmentVariable(string sEnvName)
        {
            string sRet = string.Empty;
            try
            {
                IntPtr ptrResult = STGetEnvironmentvariable(sEnvName);
                if (ptrResult != IntPtr.Zero)
                {
                    sRet = Marshal.PtrToStringAuto(ptrResult);
                }
            ...
            {
                cGlobalSettings.oLogger.WriteLogException("GetEnvironmentVariable ::", oEx);
                //  oUtils.WriteToFile(string.Format("GetEnvironmentVariable()|error:{0}", oEx.Message));
            }
            return sRet;
        }
