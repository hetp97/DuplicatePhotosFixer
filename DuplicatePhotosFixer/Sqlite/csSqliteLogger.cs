...
namespace DuplicatePhotosFixer.Sqlite
{
        ...
        public partial class cSqliteLogger
        {
          ...
          public  void InsertNewRecordsToDB(IEnumerable<csImageFileInfo> fileIdsNotFoundInDb)
            {
                try
                {
                    /// insert these to db
                    ...
                    foreach (csImageFileInfo i in fileIdsNotFoundInDb)
                    {
                        if (cGlobalSettings.listImageFileInfo[i.key].exifInfo != null && cGlobalSettings.listImageFileInfo[i.key].exifInfo.FileExifId < 0)
                        {
                            DataRow dr = cGlobalSettings.sqliteLogger.odfExifInfo.oDsUpdate.Tables[0].NewRow();
                            ConvertImageFileInfoToDataRow(cGlobalSettings.listImageFileInfo[i.key], ref dr);
                    ...
                    foreach (csImageFileInfo i in fileIdsNotFoundInDb)
                    {
                        if (cGlobalSettings.listImageFileInfo[i.key].exifInfo != null)
                        {
                            DataRow dr = cGlobalSettings.sqliteLogger.odfHash.oDsUpdate.Tables[0].NewRow();

                            dr[SqliteEnums.HashTableColumnsNames.FileExifID.ToString()] = cGlobalSettings.listImageFileInfo[i.key].exifInfo.FileExifId;

                            //if (cGlobalSettings.isSimilarMatchChecked)
                            {
                                dr[SqliteEnums.HashTableColumnsNames.FileHash.ToString()] = cGlobalSettings.listImageFileInfo[i.key].byteHash;
                            }
                            /*else
                            {
      ...
      public  void UpdateRecordsToDB(IEnumerable<csImageFileInfo> fileIdsToUpdateInDb)
        {
            try
            {
                foreach (csImageFileInfo i in fileIdsToUpdateInDb)
                {

                    Double CaptureDateSeconds = 0;
                    if (cGlobalSettings.listImageFileInfo[i.key].exifInfo.CaptureDate == DateTime.MinValue || cGlobalSettings.listImageFileInfo[i.key].exifInfo.CaptureDate == DateTime.MaxValue)
                    {
                        CaptureDateSeconds = DateTime.MaxValue.Subtract(new DateTime(1970, 1, 1)).TotalSeconds;
                    }
                    else
                        CaptureDateSeconds = cGlobalSettings.listImageFileInfo[i.key].exifInfo.CaptureDate.Subtract(new DateTime(1970, 1, 1)).TotalSeconds;

                    Double ModifiedDateSeconds = 0;
                    if (cGlobalSettings.listImageFileInfo[i.key].modDate == DateTime.MinValue || cGlobalSettings.listImageFileInfo[i.key].modDate == DateTime.MaxValue)
                    {
                        ModifiedDateSeconds = DateTime.MaxValue.Subtract(new DateTime(1970, 1, 1)).TotalSeconds;
                    }
                    else
                        ModifiedDateSeconds = cGlobalSettings.listImageFileInfo[i.key].modDate.Subtract(new DateTime(1970, 1, 1)).TotalSeconds;
                    ...
                    cGlobalSettings.sqliteLogger.odfExifInfo.updateData(SqliteEnums.MediaDatabaseLogType.ExifInfo.ToString(),
                        /*FieldName*/   new string[] { SqliteEnums.ExifInfoColumnsNames.CaptureDate.ToString(), SqliteEnums.ExifInfoColumnsNames.DpiHeight.ToString(), SqliteEnums.ExifInfoColumnsNames.DpiWidth.ToString(), SqliteEnums.ExifInfoColumnsNames.FileSize.ToString(), SqliteEnums.ExifInfoColumnsNames.GpsLatitude.ToString(), SqliteEnums.ExifInfoColumnsNames.GpsLongitude.ToString(), SqliteEnums.ExifInfoColumnsNames.modDate.ToString(), SqliteEnums.ExifInfoColumnsNames.Orientation.ToString(), SqliteEnums.ExifInfoColumnsNames.PixelHeight.ToString(), SqliteEnums.ExifInfoColumnsNames.PixelWidth.ToString(),
                                            SqliteEnums.ExifInfoColumnsNames.SharpnessScore.ToString(),  SqliteEnums.ExifInfoColumnsNames.FaceScore.ToString(),SqliteEnums.ExifInfoColumnsNames.EyeOpenScore.ToString(),
                                            SqliteEnums.ExifInfoColumnsNames.NoiseScore.ToString(),SqliteEnums.ExifInfoColumnsNames.ResolutionScore.ToString(), SqliteEnums.ExifInfoColumnsNames.PhotoScore.ToString()},
                        new byte[] { 4, 4, 4, 4, 2, 2, 4, 1, 4, 4, 6, 6, 6, 6, 6, 6 },
                        new object[]{
                            CaptureDateSeconds/*cGlobalSettings.listImageFileInfo[i].exifInfo.CaptureDate*/
                            ,cGlobalSettings.listImageFileInfo[i.key].exifInfo.VerticalResolution
                            ,cGlobalSettings.listImageFileInfo[i.key].exifInfo.HorizontalResolution
                            ,cGlobalSettings.listImageFileInfo[i.key].fileSize
                            ,cGlobalSettings.listImageFileInfo[i.key].exifInfo.latitudeDegree
                            ,cGlobalSettings.listImageFileInfo[i.key].exifInfo.longitudeDegree
                            ,ModifiedDateSeconds/*cGlobalSettings.listImageFileInfo[i].fileInfo.LastWriteTime*/
                            ,cGlobalSettings.listImageFileInfo[i.key].exifInfo.Orientation
                            ,cGlobalSettings.listImageFileInfo[i.key].exifInfo.ImageDimension.Height
                            ,cGlobalSettings.listImageFileInfo[i.key].exifInfo.ImageDimension.Width
                            ,cGlobalSettings.listImageFileInfo[i.key].Sharpness
                            ,cGlobalSettings.listImageFileInfo[i.key].FaceScore
                            ,cGlobalSettings.listImageFileInfo[i.key].EyeOpenScore
                            ,cGlobalSettings.listImageFileInfo[i.key].NoiseScore
                            ,cGlobalSettings.listImageFileInfo[i.key].ResolutionScore
                            ,cGlobalSettings.listImageFileInfo[i.key].PhotoScore
                        },
                        string.Format(" {0}={1} ", SqliteEnums.ExifInfoColumnsNames.Id.ToString(), cGlobalSettings.listImageFileInfo[i.key].exifInfo.FileExifId),
                        new string[] { SqliteEnums.ExifInfoColumnsNames.Id.ToString() },
                        new byte[] { 4 },
                        new object[] { cGlobalSettings.listImageFileInfo[i.key].exifInfo.FileExifId });
                    ...
                    cGlobalSettings.sqliteLogger.odfExifInfo.deleteData(tableName, string.Format(" {0}={1} ", SqliteEnums.HashTableColumnsNames.FileExifID.ToString(), cGlobalSettings.listImageFileInfo[i.key].exifInfo.FileExifId), new string[] { SqliteEnums.HashTableColumnsNames.FileExifID.ToString() },
                        new byte[] { 4 },
                        new string[] { cGlobalSettings.listImageFileInfo[i.key].exifInfo.FileExifId.ToString() });
                }
            }
            catch (Exception ex)
            {
