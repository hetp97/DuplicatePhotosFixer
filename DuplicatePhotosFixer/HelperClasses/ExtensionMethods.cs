...
namespace DuplicatePhotosFixer.HelperClasses
{
    public static class ExtensionMethods
    {
        ...
        public static List<ConcurrentDictionary<T1, T2>> Split<T1, T2>(this ConcurrentDictionary<T1, T2> source, int groupSize)
        {
            return source
                .Select((x, i) => new { Index = i, Value = x })
                .GroupBy(x => x.Index / groupSize/*3*/)
                .Select(x => new ConcurrentDictionary<T1, T2>(x.Select(v => v.Value).ToDictionary(z => z.Key, z => z.Value)))
                .ToList();
        }
