...
namespace DuplicatePhotosFixer
{
    ...
    internal static class ThumbnailExtractor
    {
        ...
        public static Image FromFile(string filename, Size size, UseEmbeddedThumbnails useEmbeddedThumbnails, bool useExifOrientation, bool useWIC, out PropertyItem[] exifInfo)
        {
            Image image = null;
            exifInfo = null;
            ...
            if (useWIC)
            {
                ...
                try
                {
                    using (FileStream stream = File.Open(filename, FileMode.Open, FileAccess.Read, FileShare.Read))
                    {
                        ...
                        /*DuplicatePhotoFixer.ClassDictionary.csExifInfo oExifInfo = new DuplicatePhotoFixer.ClassDictionary.csExifInfo(keyNode, image);

                        DuplicatePhotoFixer.App_Code.cGlobalSettings.listImageFileInfo[keyNode].exifInfo = oExifInfo;*/
                ...
                catch
                {
                    ...
                    return GetThumbnailBmp(filename, size, useEmbeddedThumbnails,
                        useExifOrientation ? GetRotation(filename) : 0, out exifInfo);
                }
                finally
                {
                    //if (image != null) image.Dispose(); image = null;
...
#else
                // .Net 2.0 fallback
                return GetThumbnailBmp(filename, size, useEmbeddedThumbnails, useExifOrientation ? GetRotation(filename) : 0, out exifInfo);
#endif
            }
            else
            {
            ...
            if (useWIC)
            {
                ...
                try
                {
                    using (FileStream stream = File.Open(filename, FileMode.Open, FileAccess.Read, FileShare.Read))
                    {
                        ...
                        /*if (DuplicatePhotoFixer.App_Code.cGlobalSettings.listImageFileInfo[keyNode].exifInfo == null)
                        {
        ...
        /// <summary>
        /// VG170221 - Creates a thumbnail from the given image file.
        /// </summary>
        /// <comment>
        /// This much faster .NET 3.0 method replaces the original .NET 2.0 method.
        /// The image quality is slightly reduced (low filtering mode).
        ...
        /// <param name="useWIC">true to use Windows Imaging Component; otherwise false.</param>
        /// <returns>The thumbnail image from the given file or null if an error occurs.</returns>
        public static Image FromFile(string filename, Size size, UseEmbeddedThumbnails useEmbeddedThumbnails, bool useExifOrientation, bool useWIC, string ThumbnailPath)
        {
            if (string.IsNullOrEmpty(filename))
                throw new ArgumentException("Filename cannot be empty", "filename");
            ...
            if (useWIC)
            {
                ...
                try
                {
                    ...
                    {
                        using (FileStream stream = File.Open(ThumbnailPath, FileMode.Open, FileAccess.Read, FileShare.Read))
                        {
        ...
        /// <param name="rotate">Rotation angle.</param>
        /// <returns>The image from the given file or null if an error occurs.</returns>
        internal static Image GetThumbnailBmp(string filename, Size size, UseEmbeddedThumbnails useEmbeddedThumbnails, int rotate, out PropertyItem[] exifInfo)
        {
            exifInfo = null;
            if (size.Width <= 0 || size.Height <= 0)
                throw new ArgumentException();
        ...
        private static int GetRotation(BitmapFrame frameWpf)
        {
            ...
            if (data != null)
            {
                try
                {
                    ...
                    if (obj == null)
                        return 0;
                    ushort orientationFlag = Convert.ToUInt16(obj);
