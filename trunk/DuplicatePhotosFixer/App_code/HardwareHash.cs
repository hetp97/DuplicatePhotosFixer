...
using DuplicatePhotosFixer.Trial_Protection.Engine;
...
namespace DuplicatePhotosFixer.App_code
{
    ...
    class NetworkCards
    {
        ...
        public static List<string> GetOriginalMacAddresses()
        {

            List<string> macAddress = null;
            try
            {
                string query = "SELECT * FROM Win32_NetworkAdapter WHERE (MACAddress IS NOT NULL) AND (NOT (PNPDeviceID LIKE 'ROOT%')) AND (NOT (PNPDeviceID LIKE 'USB%'))";
                string WMI_FIELD_PNPDeviceID = "PNPDeviceID";
                string WMI_FIELD_Caption = "Caption";
                string WMI_FIELD_Description = "Description";
                string WMI_FIELD_PhysicalAdapter = "PhysicalAdapter";
                string WMI_FIELD_SystemName = "SystemName";
                string WMI_FIELD_MACAddress = "MACAddress";
                string WMI_FIELD_PermanentAddress = "PermanentAddress";
                string WMI_FIELD_ProductName = "ProductName";
                    ...
                    using (ManagementObjectCollection moCollection = moSearch.Get())
                    {
                        foreach (ManagementObject mo in moCollection)
                        {
                            ...
                            try
                            {
                                ...
                                //string value = mo.Properties[PNPDeviceID].Value.ToString().ToLower().Trim();

                                string PNPDeviceID = cUtilityFunctions.ReadWmiValue(mo, WMI_FIELD_PNPDeviceID, string.Empty).ToLower().Trim();

                                try
                                {
                                    /// log hash details                                    
                                    string SystemName = cUtilityFunctions.ReadWmiValue(mo, WMI_FIELD_SystemName, string.Empty);
                                    string Caption = cUtilityFunctions.ReadWmiValue(mo, WMI_FIELD_Caption, string.Empty);
                                    string Description = cUtilityFunctions.ReadWmiValue(mo, WMI_FIELD_Description, string.Empty);
                                    string ProductName = cUtilityFunctions.ReadWmiValue(mo, WMI_FIELD_ProductName, string.Empty);
                                    string PhysicalAdapter = cUtilityFunctions.ReadWmiValue(mo, WMI_FIELD_PhysicalAdapter, string.Empty);
                                    string MACAddress = cUtilityFunctions.ReadWmiValue(mo, WMI_FIELD_MACAddress, string.Empty);
                                    string PermanentAddress = cUtilityFunctions.ReadWmiValue(mo, WMI_FIELD_PermanentAddress, string.Empty);
                                    ...
                                    sb.Append(string.Format("\r\nPhysicalAdapter:{0}", PhysicalAdapter));
                                    sb.Append(string.Format("\r\nMACAddress:{0}", MACAddress));
                                    sb.Append(string.Format("\r\nPermanentAddress:{0}", PermanentAddress));
                                    sb.Append(string.Format("\r\nPNPDeviceID:{0}", PNPDeviceID));
                                    sb.Append("\r\n============");
                                    cGlobalSettings.oLogger.WriteLogVerbose(sb.ToString());
                                    sb.Remove(0, sb.Length - 1);
                                    sb = null;
                                ...
                                {
                                    cGlobalSettings.oLogger.WriteLogException("NetworkCards::GetOriginalMacAddresses", ex);
                                }


                                if (!string.IsNullOrEmpty(PNPDeviceID))
                                {
                                    string mac = GetPhysicalAddr(PNPDeviceID, true);
                                    if (!string.IsNullOrEmpty(mac))
                                    {
                                        cGlobalSettings.oLogger.WriteLogVerbose("NetworkCards::GetOriginalMacAddresses:Physical Adapter found with Mac Address:{0}", mac);
                                        macAddress.Add(mac.Replace("-", ""));
                                    }
                                }
        ...
        private static string GetPhysicalAddr(string cardId, bool bModifyCardId = false)
        {
            string macAddress = string.Empty;
            uint device = 0;
            try
            {
                string driveName = "\\\\.\\" + (bModifyCardId ? (cardId + ("#{ad498944-762f-11d0-8dcb-00c04fc3358c}")).Replace("\\", "#") : cardId);
        ...
        public static string GetMacAddr()
        {
            ...
            foreach (var netcard in System.Net.NetworkInformation.NetworkInterface.GetAllNetworkInterfaces())
            {
    ...
    public class CRC64CPP : HashAlgorithm
    {
        ...
        private static readonly UInt64[] table = new UInt64[] {
                ...
                (0x2c57b3b8eb0bDFF5), (0x6ea7525342e1e956),
        ...
        const UInt64 INITIALCRC64 = 0xFFFFFFFFFFFFFFFF;
        private static UInt64 CalculateHash(UInt64 seed, byte[] buffer, int start, int size)
        {
            UInt64 crc = INITIALCRC64/*seed*/; //18446744073709551615

            for (int i = start; i < size; i++)
                unchecked
                {
                    //crc = (crc >> 8) ^ table[(buffer[i] ^ crc) & 0xff]; // actual
                    // crc = table[(char)(crc >> 56) ^ buffer[i]++] ^ (crc << 8);

                    uint tableIndex = (((uint)(crc >> 56)) ^ buffer[i]) & 0xff;

                    crc = table[tableIndex] ^ (crc << 8);
                }

            return (crc ^ INITIALCRC64);
        }
