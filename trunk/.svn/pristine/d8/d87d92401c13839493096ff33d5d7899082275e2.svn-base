...
namespace DuplicatePhotosFixer.Engine.WinApiScan
{
    public static class NativeMethods
    {
        ...
        [DllImport("kernel32.dll", SetLastError = true)]
        public static extern int CloseHandle(IntPtr hModule);

        [DllImport("kernel32.dll")]
        public static extern uint GetLastError();

        [DllImport("kernel32.dll", SetLastError = true)]
        public static extern bool InitializeCriticalSectionAndSpinCount(ref IntPtr lpCriticalSection, uint dwSpinCount);

        [DllImport("kernel32.dll")]
        public static extern IntPtr CreateThread(IntPtr SecurityAttributes, uint StackSize, IntPtr StartFunction, IntPtr ThreadParameter, uint CreationFlags, out uint ThreadId);
        ...
        private static extern bool AdjustTokenPrivileges(IntPtr TokenHandle, [MarshalAs(UnmanagedType.Bool)] bool DisableAllPrivileges, ref TOKEN_PRIVILEGES NewState, UInt32 BufferLengthInBytes, out TOKEN_PRIVILEGES PreviousState, out UInt32 ReturnLengthInBytes);
        ...
        public static void PreventSystemPowerEventSleep()
        {
            NativeMethods.SetThreadExecutionState(NativeMethods.ES_DISPLAY_REQUIRED | NativeMethods.ES_SYSTEM_REQUIRED | NativeMethods.ES_CONTINUOUS);
        }
        ...
        public class NativeFileMethods
        {
            #region "fast win32 file scan"
            ...
            public static bool CheckDirectoryEmpty_Fast(string path, bool bLookForDir)
            {
                try
                {
                        ...
                        if (findHandle != INVALID_HANDLE_VALUE)
                        {
                            try
                            {
                                ...
                                do
                                {
                                    ...
                                    {
                                        string filePath = PathEx.Combine(searchDir, findData.cFileName).ToLower();
                                        if (bLookForDir)
                                        {
                                            if ((findData.dwFileAttributes & FileAttributes.Directory) == FileAttributes.Directory)
                                            {
                                                if (!IsDirectoryAttributesIgnored(filePath, findData.dwFileAttributes)
                                                     && !(filePath.Length > 248))
                                                {
                                                    empty = false;
                                                }
                                        ...
                                        else
                                        {
                                            if (!((findData.dwFileAttributes & FileAttributes.Directory) == FileAttributes.Directory))
                                            {
                                                /*if (!cGlobalSettings.scannerRules.IsFileAttributesIgnored(filePath, findData.dwFileAttributes) && !(filePath.Length > 259)
                                                    && !cGlobalSettings.scannerRules.ignoreRules.IgnoreFileHardPattern.IsMatch(findData.cFileName.ToLower())
                                                    && !cGlobalSettings.scannerRules.ignoreRules.IgnoreExtensionHardPattern.IsMatch(findData.cFileName.ToLower()))*/
                                                {
                                                    empty = false;
                                                }
                                            }
                ...
                catch (System.Exception ex)
                {
                    ...
                    cGlobalSettings.oLogger.WriteLogException("CheckDirectoryEmpty_Fast", ex);
                }
                return true;
                //throw new DirectoryNotFoundException();
            }

            public static bool IsDirectoryAttributesIgnored(string FullName, FileAttributes Attributes)
            {
                return (
        ...
        public static int CloseDirectoryHandle(IntPtr handle)
        {
